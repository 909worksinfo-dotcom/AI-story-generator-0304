...
# 极其严格的格式规范
def get_strict_format(num):
    return f"""
[输出结构要求]
请写出 {num} 个剧本创意，每个创意必须严格遵循以下 5 行结构：
第1行：英文剧名《中文剧名》
...
with st.sidebar:
    st.header("⚙️ 引擎配置")
    provider = st.selectbox("选择厂商", ["OpenRouter","Gemini", "GPT (OpenAI)", "Claude (Anthropic)"])
    models_map = {
...
with col_l:
    ...
    theme_custom = st.text_input("输入其他题材", placeholder="填入将替换主题", height=50)
    active_theme = theme_custom if theme_custom else theme_btn

    # 【新增】用户自定义生成数量，默认值为 30
    num_ideas = st.number_input("期望生成创意数量", min_value=5, max_value=100, value=30, step=5)

    # --- A. 立即生成按钮 (动态数量) ---
    if st.button(f"🚀 立即生成创意 ({num_ideas}个)", use_container_width=True):
        if not api_key:
            st.error("请先输入 API Key")
        else:
            with st.spinner(f"正在创作 {num_ideas} 个创意..."):
                strict_format = get_strict_format(num_ideas)
                p1 = f"{IDENTITY}\n任务：写 {num_ideas} 个{active_theme}短剧创意。\n{strict_format}"
                draft = call_ai_engine(provider, api_key, model_name, p1)

                if draft and "❌" not in draft:
                    time.sleep(1)
                    p2 = f"作为严格的审查官，确保以下 {num_ideas} 个创意符合五行结构，每项必须包含“概括：”前缀。只输出内容：\n{draft}\n{strict_format}"
                    ...
                    # 【新增】记录当前成功生成的数量，供后续“精修”和“筛选”使用
                    st.session_state['current_num_ideas'] = num_ideas
    ...
    if 'res' in st.session_state:
        current_num = st.session_state.get('current_num_ideas', 30)  # 获取当前批次的数量
        st.write("---")
        st.write(f"🔧 针对性精修 (同步修改 {current_num} 个创意):")
        ...
        if st.button("📊 筛选前 5 名 (完整展示)", use_container_width=True):
            with st.spinner("正在挑选 Top 5 爆款潜力作品并准备完整内容..."):
                ...
                作为短剧制片人，请从以下 {current_num} 个创意中评估其爆款潜力，选出得分最高的 5 个。
...
with col_r:
    ...
    if 'res' in st.session_state:
        current_num = st.session_state.get('current_num_ideas', 30)
        st.write(f"📋 原始生成的 {current_num} 个创意全量列表：")
        st.text_area(f"全量 {current_num} 个预览", st.session_state['res'], height=800)
    else:
        st.info("💡 请在左侧生成创意。")
